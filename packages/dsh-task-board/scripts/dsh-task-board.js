...
const PKG = '@captain1275/dsh-client-ui-task-board'
...
function ensureProfile() {
  if (!fs.existsSync(PROFILE_MANIFEST)) {
    throw new Error(`web profile manifest 不存在：${PROFILE_MANIFEST}（先运行 dsh web —dsh plugin --profile web 初始化）`)
  }
}
...
function unmount() {
  ...
  if (deps[PKG] !== undefined) {
    ...
    console.log(`· dependencies —${PKG}（跳过）`)
  }
  ...
  if (idx !== -1) {
    ...
    console.log(`· dsh.profile.bundles —${PKG}（跳过）`)
  }
  if (changed) {
    writeManifest(manifest)
    installProfile()
...
function status() {
  ...
  console.log(`bundles     : ${bundles.includes(PKG) ? '已列出dsh.profile.bundles' : '未列出}`)

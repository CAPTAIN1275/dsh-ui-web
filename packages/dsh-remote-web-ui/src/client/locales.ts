...
export const zh = {
  ...
  'update.linkModeDetail': 'dsh-web-ui 通过本地链接安装（开发模式），无法自动更新。当前版本：v{current}，npm 上最新版本：{latest}。请在仓库中执行 git pull 更新。',
  'update.missing': '未找到 dsh-web-ui 安装',
  'update.missingDetail': '无法定位已安装的 dsh-web-ui 聚合包。请先运行 dsh plugin --profile <profile> add @captain1275/dsh-web-ui-all。',
...
export const en = {
  ...
  'update.linkModeDetail': 'dsh-web-ui is installed via a local link (development mode) and cannot be updated remotely. Current version: v{current}, latest npm release: {latest}. Run git pull in the repository to update.',
  'update.missing': 'dsh-web-ui not found',
  'update.missingDetail': 'Could not locate the installed dsh-web-ui aggregate package. Install it with: dsh plugin --profile <profile> add @captain1275/dsh-web-ui-all',

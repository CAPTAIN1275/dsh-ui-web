...
*/
/**
* Walk up from a file location to the nearest @captain1275/ scoped dir
...
* '../../' from the skin-center package can never see its siblings there — * this anchor finds the scoped dir that owns them.
...
function findScopedAnchor(fromDir) {
	let current = fromDir;
	for (;;) {
		const scoped = join(current, "@captain1275");
		try {
...
*    (packages/skins/ or node_modules/@captain1275/);
*  - pnpm virtual-store layout: the nearest @captain1275/ scoped dir found by
...
*    and per-skin npm packages @captain1275/dsh-client-ui-skin-<id>);
*  - the bundled-skins carrier: @captain1275/dsh-skins/skins/<id> (skin assets
...
* also matches the current @captain1275 scope so stale rows are always cleaned.
...
* layout where the skin package sits at node_modules/@captain1275/..., or a
...
function checkResolvable(entry, profileModulesDir) {
	...
	if (!statSync(pkgPath, { throwIfNoEntry: false })) return `${entry.pkg} —profile 中缺少 package.json—{pkgPath}）——聚合包皮肤目录未带可解析包元数据。`;
	let parsed;
	try {
...
* @module @captain1275/dsh-client-ui-skin-center/routes
*/
...
* @param args - command arguments (e.g. `['use', 'ths']`).
*/
...
* node_modules/@captain1275/) and candidates include the bundled dsh-skins

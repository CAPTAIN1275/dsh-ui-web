...
- **通信**：浏览器 ↔ host 走同源 `/api/pet/*` JSON 端点（state/interact/set-visible/set-config），图集从 `/pet/whale/spritesheet.webp` 加载——RPC 域与 `/plugins/` 静态服务都是平台注册的，插件自足地提供自己的 API 与素材（与 live-stats / usage-dashboard 的宿主路由同一模式）。
...
```sh
# 推荐：直接从 npm 安装
dsh plugin --profile web add @captain1275/dsh-pet
